...
1. **`4005c90` — `modulos/produccion-gestion.html` con el diseño "Producción · Gestión"** (handoff de Claude Design, compu 1440 y celular 390).
...
### 1. La gestión con el diseño nuevo (`4005c90`)
...
- **Se EJECUTA el código real** (`pruebas/sandbox-produccion.js`) con datos de prueba y HTML malicioso en cada render nuevo. Suites, al cerrar:
  ...
  - `controles-produccion-gestion.js` **612/612** (con `RETIRADOS` y el baseline nuevo `4005c90` en `BASES_GESTION`) · `controles-produccion.js` 1722/1722 · `check-scripts` OK · `check-bytes` 0 CR, 0 NUL · **`correr-todo.js` 98/98**.
...
> - `controles-produccion-gestion.js` tiene `RETIRADOS` (con su motivo) y el baseline `4005c90`. Suites nuevas: `test-/mut-produccion-gestion-diseno.js` y `test-/mut-produccion-stock-retiros.js`.

...
correrMutaciones({
  ...
  manuales: [
    ...
    { nombre: 'la cartera suma sin pasar a centavos',
      de: '        cantidad++\n        centavos += Math.round(Number(ch.importe) * 100)\n      }\n      return { cantidad, total: centavos / 100 }', a: '        cantidad++\n        centavos += Number(ch.importe) * 100\n      }\n      return { cantidad, total: centavos / 100 }' },
    ...
    { nombre: 'Dar salida se ofrece sin la tarea procesar',
      de: "if (ch.estado !== 'en_cartera' || !puedeProcesar()) return null", a: "if (ch.estado !== 'en_cartera') return null" },
    { nombre: 'Volver a cartera se ofrece sin la tarea procesar',
      de: "if (salido) return puedeProcesar() ? 'volver' : null", a: "if (salido) return 'volver'" },
    { nombre: 'la cobranza por controlar no dice por qué no hay botón',
      de: "      if (cob?.estado === 'registrada') return 'nota'", a: "      if (false) return 'nota'" },
    ...
    { nombre: 'las filas dejan de tener el mismo alto',
      de: '    .chq-tabla tbody td { height: 3.5rem; vertical-align: middle; padding-top: 0.375rem; padding-bottom: 0.375rem; }\n', a: '' },
    { nombre: 'un cliente largo vuelve a bajar de renglón',
      de: '    .chq-tabla__texto { max-width: 10rem; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }', a: '    .chq-tabla__texto { min-width: 11rem; overflow-wrap: anywhere; }' },
    ...
    { nombre: 'el destino vacío viaja como ""',
      de: 'p_cheque_id: chequeId, p_tipo: tipo, p_fecha: fecha, p_destino: textoOpcional(destino) }', a: 'p_cheque_id: chequeId, p_tipo: tipo, p_fecha: fecha, p_destino: destino }' },
    ...
    { nombre: 'el diálogo describe el cheque con innerHTML',
      de: "document.getElementById('chq-salida-cheques').textContent =\n        `N° ${ch.numero", a: "document.getElementById('chq-salida-cheques').innerHTML =\n        `N° ${ch.numero" },

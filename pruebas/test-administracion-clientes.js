...
// ── El alta ────────────────────────────────────────────────────────────────
{
  ...
  preparar(S)
  esperas.push(S.mostrarClientes().then(() => {
    chk('solo con retiros:precios NO hay alta, y se dice por qué', S.__els.get('ad-btn-cliente-nuevo').hidden === true && S.__els.get('ad-clientes-alta-aviso').hidden === false)
    S.abrirAlta()
    chk('y no se abre', S.estado.alta === null)
  }))
  const T = nuevo()
  preparar(T)
